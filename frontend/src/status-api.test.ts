...
import {approveVersion,archiveTask,createTaskVersion,returnToDraft,submitReview} from "./api";
...
describe("status API",()=>{
 ...
 it("creates from the approved latest version and exposes Location",async()=>{const fetch=vi.spyOn(globalThis,"fetch").mockResolvedValue(new Response("{}",{status:201,headers:{Location:"/api/content-bank/tasks/task"}}));const result=await createTaskVersion("task",4);expect(JSON.parse(String(fetch.mock.calls[0][1]?.body))).toEqual({source_version_no:4});expect(result.location).toBe("/api/content-bank/tasks/task")});

...
export type AuthoringWorkspaceViewV1={schema_version:"authoring_workspace_view.v1";session:{id:string;lifecycle_status:string;created_at:string;owner:string};generation:{generated_draft_available:boolean;generator_route:Record<string,unknown>|null;generator_attempt_status:string|null};solver:{solver_status:string|null;validation_status:string|null;semantic_result:Record<string,unknown>|null};review:{current_revision:number|null;revision_count:number;reviewer_state:string;latest_changes_summary:Record<string,unknown>};quality:{quality_status:"not_available"|"passed"|"warnings"|"blocked";blocking_findings:{code:string;message:string}[];warnings:{code:string;message:string}[];override_state:{overridden:boolean;reason:string|null;warning_codes:string[]}};acceptance:{accepted_revision:{id:string;revision_number:number}|null;promoted_task_reference:{task_id:string;task_version_id:string}|null};diagnostics:{attempt_count:number;total_cost_by_currency:{currency:string;amount:string}[];provider_usage_summary:{provider_id:string;model_id:string;role:string;attempt_count:number;input_tokens:number;output_tokens:number;cache_read_tokens:number;cache_write_tokens:number}[]}};
export const getAuthoringWorkspace=(id:string,signal?:AbortSignal):Promise<AuthoringWorkspaceViewV1>=>request(`/api/content-bank/authoring/sessions/${encodeURIComponent(id)}/workspace`,{signal});
export type AuthoringRoute={provider_id:string;model_id:string};
export type AuthoringCreateRequest={subject_id:string;grade_id:string;topic_id:string;subtopic_id:string|null;skill_ids:string[];task_goal:string;task_type:"test"|"calculation"|"problem"|"open_question"|"essay";answer_format:"single_choice"|"multiple_choice"|"short_text"|"number"|"expression"|"long_text";difficulty:number;pedagogical_constraints:string[]};
export type AuthoringSessionResponse={id:string;execution_status:string};
export const getAuthoringRoutes=(signal?:AbortSignal):Promise<{items:AuthoringRoute[]}>=>(request("/api/content-bank/authoring/routes",{signal}));
export const createAuthoringSession=(value:AuthoringCreateRequest):Promise<AuthoringSessionResponse>=>request("/api/content-bank/authoring/sessions",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(value)});
export const runAuthoringSession=(id:string,generator_route:AuthoringRoute,solver_route:AuthoringRoute):Promise<AuthoringSessionResponse>=>request(`/api/content-bank/authoring/sessions/${encodeURIComponent(id)}/run`,{method:"POST",headers:{"Content-Type":"application/json","Idempotency-Key":`authoring-ui-${id}`},body:JSON.stringify({generator_route,solver_route})});
export type AuthoringAcceptanceRequest={acceptance_note?:string;confirm_questionable?:boolean;warning_override_reason?:string};
export type AuthoringAcceptanceResponse={session_id:string;task_id:string;task_version_id:string;created_at:string;lifecycle_status:"draft"|"review"|"approved"|"archived";already_existing:boolean};
export const acceptAuthoringSession=(id:string,value:AuthoringAcceptanceRequest):Promise<AuthoringAcceptanceResponse>=>request(`/api/content-bank/authoring/sessions/${encodeURIComponent(id)}/accept`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(value)});

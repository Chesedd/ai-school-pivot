...
export function ValidationIssueList({title,issues,kind="error",description}:{title:string;issues:ValidationIssue[];kind?:"warning"|"error";description?:string}){
 return <section className={`validation-report ${kind}`} role={kind==="error"?"alert":undefined} aria-live={kind==="warning"?"polite":undefined}><h2>{title}</h2>{description&&<p>{description}</p>}{issues.length?<ul>{issues.map((x,i)=><li key={`${x.field}-${x.code}-${i}`}><strong>{x.message}</strong>{x.field&&<span>Поле: {x.field}</span>}{x.code&&<small>Код: {x.code}</small>}</li>)}</ul>:<p>Замечаний нет.</p>}</section>
}
...
const descriptions:Record<string,string>={draft:"Черновик можно отправить на проверку после сохранения методики.",review:"Задание можно утвердить или вернуть автору на доработку.",approved:"Утверждённое задание доступно только для чтения.",archived:"Архивная карточка доступна только для чтения."};

...
export function StatusActionBar({status,archived,dirty,busy,methodologySaving,onAction,allowed=["submit","return","approve","archive"]}:{status:string;archived:boolean;dirty:boolean;busy:StatusAction|null;methodologySaving:boolean;onAction:(action:StatusAction)=>void;allowed?:StatusAction[]}){
 ...
 return <section className="status-bar" aria-labelledby="status-title"><div><h2 id="status-title">Статус задания</h2><p><span className="badge">{status}</span></p>{archived&&<p>Архивная карточка доступна только для чтения.</p>}{dirty&&<p className="dirty-hint">Сначала сохраните или отмените изменения методической структуры</p>}{methodologySaving&&<p>Дождитесь завершения сохранения методики.</p>}</div>{!archived&&status!=="archived"&&<div className="status-buttons">{status==="draft"&&allowed.includes("submit")&&button("submit","Отправить на проверку","Отправка на проверку…")}{status==="review"&&allowed.includes("return")&&button("return","Вернуть в черновик","Возврат в черновик…","secondary")}{status==="review"&&allowed.includes("approve")&&button("approve","Утвердить","Утверждение…")}{allowed.includes("archive")&&button("archive","Архивировать задание","Архивирование…","danger")}</div>}</section>
}

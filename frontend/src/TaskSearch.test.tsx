...
describe("Content Bank search",()=>{
 ...
 it("resets offset for a new query",async()=>{setup(vi.spyOn(globalThis,"fetch").mockResolvedValue(response(Array.from({length:25},(_,i)=>({...item,task_id:`task-${i}`})),25)));await loaded();await userEvent.click(screen.getByRole("button",{name:"Далее"}));await waitFor(()=>expect(requests().at(-1)!.searchParams.get("offset")).toBe("20"));await submit("новый");await waitFor(()=>expect(requests().at(-1)!.searchParams.get("offset")).toBe("0"))});
 it("preserves q on the next page",async()=>{setup(vi.spyOn(globalThis,"fetch").mockResolvedValue(response(Array.from({length:25},(_,i)=>({...item,task_id:`task-${i}`})),25)));await loaded();await submit("движение");await waitFor(()=>expect(requests().at(-1)!.searchParams.get("q")).toBe("движение"));await userEvent.click(screen.getByRole("button",{name:"Далее"}));await waitFor(()=>{const p=requests().at(-1)!.searchParams;expect(p.get("q")).toBe("движение");expect(p.get("offset")).toBe("20")})});
 ...
 it("Clear removes q and resets offset",async()=>{setup(vi.spyOn(globalThis,"fetch").mockResolvedValue(response(Array.from({length:25},(_,i)=>({...item,task_id:`task-${i}`})),25)));await loaded();await submit("движение");await userEvent.click(screen.getByRole("button",{name:"Далее"}));await userEvent.click(screen.getByRole("button",{name:"Очистить"}));await waitFor(()=>{const p=requests().at(-1)!.searchParams;expect(p.has("q")).toBe(false);expect(p.get("offset")).toBe("0")})});

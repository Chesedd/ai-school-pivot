...
@pytest.mark.parametrize(("grade_number", "topic_name", "subtopic_name", "skill_name"), [
    ...
    (10, "Механика", "Второй закон Ньютона", "Применять второй закон Ньютона"),
    (10, "Молекулярная физика и термодинамика", "Уравнение Менделеева–Клапейрона", "Применять уравнение Менделеева–Клапейрона"),
    (10, "Молекулярная физика и термодинамика", "Первый закон термодинамики", "Применять первый закон термодинамики"),
    (10, "Электродинамика", "Закон Ома для полной цепи", "Применять закон Ома для полной цепи"),
    (11, "Электродинамика", "Сила Лоренца", "Вычислять силу Лоренца"),
    (11, "Электродинамика", "Закон электромагнитной индукции Фарадея", "Применять закон электромагнитной индукции Фарадея"),
    (11, "Колебания и волны", "Формула тонкой линзы", "Применять формулу тонкой линзы"),
    (11, "Квантовая физика", "Уравнение Эйнштейна для фотоэффекта", "Применять уравнение Эйнштейна для фотоэффекта"),
    (11, "Элементы астрономии и астрофизики", "Закон Хаббла", "Применять закон Хаббла в базовой расчётной задаче"),
])
...
@pytest.mark.parametrize(("grade_number", "topic_name", "subtopic_name", "skill_name"), [
    (10, "Теоретические основы информатики", "Условие Фано", "Проверять однозначность декодирования по условию Фано"),
    (10, "Теоретические основы информатики", "Таблица истинности", "Строить таблицу истинности"),
    (10, "Информационные технологии", "Библиографическая ссылка", "Оформлять библиографическую ссылку"),
    (11, "Теоретические основы информатики", "Оптимальный путь", "Находить оптимальный путь"),
    (11, "Алгоритмы и программирование", "Линейный поиск", "Выполнять линейный поиск"),
    (11, "Информационные технологии", "Реляционная база данных", "Определять ключ таблицы"),
    (11, "Информационные технологии", "Средства искусственного интеллекта", "Распознавать задачу, в которой применяется средство искусственного интеллекта"),
])
def test_informatics_10_11_catalog_metadata_resolves_locally(
        grade_number, topic_name, subtopic_name, skill_name):
    subject = item("Информатика")
    grade = item(str(grade_number), grade_number=grade_number)
    topic = item(topic_name, subject_id=subject.id, grade_id=grade.id)

...
async def db():
    ids = {key: uuid4() for key in ("actor", "group", "student", "assessment", "variant", "variant2", "item", "task", "version", "assignment", "participant", "submission")}
...
async def test_basic_rows_status_and_assessment_has_many_assignments(db):
...
async def test_variant_and_item_uniqueness_and_ranges(db):
    async with engine.begin() as c:
...
async def test_assignment_participant_and_variant_pair_constraints(db):
    now = datetime.now(timezone.utc)
    async with engine.begin() as c:
...
async def test_submission_partial_draft_unique_state_and_answers(db):
    async with engine.begin() as c:
...
async def test_idempotency_namespace_and_historical_restrict(db):
...
async def test_critical_foreign_keys_are_restrict(db):
    async with engine.begin() as c:

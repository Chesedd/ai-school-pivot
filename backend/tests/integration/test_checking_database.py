...
async def test_fk_score_unique_and_immutability_constraints(engine,seeded):
    ...
    async with engine.begin() as c:
        ...
        await rejected(c,"INSERT INTO check_results(id,check_run_id,assessment_item_id,task_version_id,checker_type,checker_version,schema_version,result_status,score_suggested,max_score,confidence,reason_code,confidence_policy_version,confidence_details,summary,needs_human_review,validated_result) VALUES (:result,:run,:missing,:version,'exact','1','1','correct',2,2,.9,'exact_match','confidence_v1','{\"effective\":\"0.9000\"}'::jsonb,'ok',false,'{}')",values)
        await c.execute(text("INSERT INTO check_results(id,check_run_id,assessment_item_id,task_version_id,checker_type,checker_version,schema_version,result_status,score_suggested,max_score,confidence,reason_code,confidence_policy_version,confidence_details,summary,needs_human_review,validated_result) VALUES (:result,:run,:item,:version,'exact','1','1','correct',2,2,.9,'exact_match','confidence_v1','{\"effective\":\"0.9000\"}'::jsonb,'ok',false,'{}')"),values)
        await rejected(c,"INSERT INTO check_results(check_run_id,assessment_item_id,task_version_id,checker_type,checker_version,schema_version,result_status,score_suggested,max_score,confidence,reason_code,confidence_policy_version,confidence_details,summary,needs_human_review,validated_result) VALUES (:run,:item,:version,'exact','1','1','incorrect',0,2,.9,'exact_mismatch','confidence_v1','{\"effective\":\"0.9000\"}'::jsonb,'x',false,'{}')",values)

...
class StudentSubmission(IdMixin, Base):
    ...
    __table_args__ = (CheckConstraint("num_nonnulls(assignment_participant_id, remediation_plan_id) = 1", name="ck_student_submissions_execution_target_xor"), CheckConstraint("attempt_no > 0", name="ck_student_submissions_attempt_positive"), CheckConstraint("(status = 'draft' AND submitted_at IS NULL) OR (status = 'submitted' AND submitted_at IS NOT NULL)", name="ck_student_submissions_status_submitted"), Index("uq_student_submissions_participant_attempt", "assignment_participant_id", "attempt_no", unique=True, postgresql_where=text("assignment_participant_id IS NOT NULL")), Index("uq_student_submissions_remediation_attempt", "remediation_plan_id", "attempt_no", unique=True, postgresql_where=text("remediation_plan_id IS NOT NULL")), Index("uq_student_submissions_one_draft", "assignment_participant_id", unique=True, postgresql_where=text("status = 'draft'")), Index("ix_student_submissions_participant_status", "assignment_participant_id", "status"), Index("ix_student_submissions_remediation_status", "remediation_plan_id", "status"), Index("ix_student_submissions_status_started", "status", "started_at"))
    assignment_participant_id: Mapped[UUID | None] = mapped_column(ForeignKey("assignment_participants.id", ondelete="RESTRICT", onupdate="RESTRICT", name="fk_student_submissions_participant_id_participants"), nullable=True); remediation_plan_id: Mapped[UUID | None] = mapped_column(ForeignKey("remediation_plans.id", ondelete="RESTRICT", onupdate="RESTRICT", name="fk_student_submissions_remediation_plan"), nullable=True); attempt_no: Mapped[int] = mapped_column(SmallInteger); status: Mapped[str] = mapped_column(submission_status, server_default=text("'draft'::submission_status"))
...
class StudentAnswer(IdMixin, Base):
    ...
    __table_args__ = (CheckConstraint("num_nonnulls(assessment_item_id, remediation_plan_item_id) = 1", name="ck_student_answers_execution_item_xor"), Index("uq_student_answers_submission_item", "submission_id", "assessment_item_id", unique=True, postgresql_where=text("assessment_item_id IS NOT NULL")), Index("uq_student_answers_submission_remediation_item", "submission_id", "remediation_plan_item_id", unique=True, postgresql_where=text("remediation_plan_item_id IS NOT NULL")), Index("ix_student_answers_assessment_item_id", "assessment_item_id"))
    submission_id: Mapped[UUID] = mapped_column(ForeignKey("student_submissions.id", ondelete="RESTRICT", onupdate="RESTRICT", name="fk_student_answers_submission_id_submissions")); assessment_item_id: Mapped[UUID | None] = mapped_column(ForeignKey("assessment_items.id", ondelete="RESTRICT", onupdate="RESTRICT", name="fk_student_answers_item_id_items"), nullable=True); remediation_plan_item_id: Mapped[UUID | None] = mapped_column(ForeignKey("remediation_plan_items.id", ondelete="RESTRICT", onupdate="RESTRICT", name="fk_student_answers_remediation_item"), nullable=True)

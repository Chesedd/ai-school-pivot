        ...
        async with session.begin():
            for number, name in GRADES:
                await session.execute(
                    text(
                        "INSERT INTO grades (number, name) VALUES (:number, :name) "
            ...
            await session.execute(
                ...
                {"code": "demo-informatics", "name": "Информатика (demo/dev)"},
            )
            ...
            await session.execute(
                text(
                    "INSERT INTO topics (subject_id, grade_id, code, name) "
                    "VALUES (:subject_id, :grade_id, :code, :name) "
                    "ON CONFLICT (subject_id, grade_id, code) DO UPDATE SET name = EXCLUDED.name"
                ),
                {"subject_id": subject_id, "grade_id": grade_id, "code": "demo-algorithms", "name": "Алгоритмы (demo/dev)"},
            )
            ...
            await session.execute(
                text(
                    "INSERT INTO subtopics (topic_id, code, name) VALUES (:topic_id, :code, :name) "
                    "ON CONFLICT (topic_id, code) DO UPDATE SET name = EXCLUDED.name"
                ),
                {"topic_id": topic_id, "code": "demo-linear-algorithms", "name": "Линейные алгоритмы (demo/dev)"},
            )
            ...
            await session.execute(
                text(
                    "INSERT INTO skills (subtopic_id, code, name) VALUES (:subtopic_id, :code, :name) "
                    "ON CONFLICT (subtopic_id, code) DO UPDATE SET name = EXCLUDED.name"
                ),
                {"subtopic_id": subtopic_id, "code": "demo-linear-program", "name": "Составление линейной программы (demo/dev)"},
            )

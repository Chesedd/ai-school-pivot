...
from app.presentation.auth_dependencies import (enforce_capability, require_any_capability,
    require_capability, require_trusted_origin)

router=APIRouter(prefix="/api", dependencies=[Depends(require_trusted_origin)])
...
async def upload_answer(submission_id:UUID,item_id:UUID,request:Request,x_filename:str=Header(...),content_type:str=Header(...),settings:Settings=Depends(get_settings),principal:Principal=Depends(require_capability(STUDENT_ATTEMPTS_SUBMIT))):
    if principal.student_id is None: raise HTTPException(403, "student_identity_required")
    async with async_session_factory() as s:
        answer=await owned_answer(s,submission_id,item_id,principal.student_id)
...
async def answer_files(submission_id:UUID,item_id:UUID,principal:Principal=Depends(require_capability(STUDENT_ATTEMPTS_SUBMIT))):
    if principal.student_id is None: raise HTTPException(403, "student_identity_required")
    async with async_session_factory() as s:
        answer=await owned_answer(s,submission_id,item_id,principal.student_id)
...
async def content(attachment_id:UUID, settings:Settings=Depends(get_settings), principal:Principal=Depends(require_any_capability(CONTENT_READ, STUDENT_ASSIGNMENTS_READ))):
    async with async_session_factory() as s:
    ...
    if student_owner is not None:
        enforce_capability(principal, STUDENT_ASSIGNMENTS_READ)

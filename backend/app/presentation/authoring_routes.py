...
async def create(payload:AuthoringCreateRequest,response:Response,svc=Depends(service),principal:Principal=Depends(require_principal)):
...
@router.get("/sessions")
async def list_sessions(offset:int=Query(0,ge=0),limit:int=Query(20,ge=1,le=100),svc=Depends(service),principal:Principal=Depends(require_principal)): return await svc.list(principal.user_id,offset,limit)
@router.get("/sessions/{session_id}",response_model=SessionResponse)
async def get(session_id:UUID,svc=Depends(service),principal:Principal=Depends(require_principal)): return await svc.get(session_id,principal.user_id)
@router.post("/sessions/{session_id}/run",response_model=SessionResponse)
async def run(session_id:UUID,payload:RunRequest,idempotency_key:str=Header(...,alias="Idempotency-Key",min_length=1,max_length=80),svc=Depends(service),principal:Principal=Depends(require_principal)):
...
async def preview(session_id:UUID,svc=Depends(service),principal:Principal=Depends(require_principal)): return await svc.preview(session_id,principal.user_id)
@router.get("/sessions/{session_id}/attempts")
async def attempts(session_id:UUID,svc=Depends(service),principal:Principal=Depends(require_principal)): return await svc.attempts(session_id,principal.user_id)

@router.get("/sessions/{session_id}/workspace", response_model=AuthoringWorkspaceViewV1)
async def workspace(session_id:UUID,db=Depends(get_session),principal:Principal=Depends(require_principal)):
...
async def start_review(session_id:UUID,db=Depends(get_session),principal:Principal=Depends(require_principal)):
...
async def get_review(session_id:UUID,db=Depends(get_session),principal:Principal=Depends(require_principal)):
...
async def review_history(session_id:UUID,db=Depends(get_session),principal:Principal=Depends(require_principal)):
...
async def review_diff(session_id:UUID,from_revision:int|None=Query(None,ge=0),
        to_revision:int|None=Query(None,ge=0),db=Depends(get_session),principal:Principal=Depends(require_principal)):
...
async def get_quality(session_id:UUID,db=Depends(get_session),principal:Principal=Depends(require_principal)):
...
async def edit_review(session_id:UUID,payload:AuthoringReviewEditRequest,db=Depends(get_session),principal:Principal=Depends(require_principal)):
...
async def reject(session_id:UUID,payload:AuthoringRejectRequest,db=Depends(get_session),principal:Principal=Depends(require_principal)):
...
async def accept(session_id:UUID,payload:AuthoringAcceptanceRequest,db=Depends(get_session),principal:Principal=Depends(require_principal)):
